...
export default class RoomsController {
  ...
  #logoutUser(socket) {
    ...
    // caso seja um usario sujeira que estava em uma sala que não existe mais
    ...
    // se nao tiver mais nenhum usuario na sala, matamos a sala
    if (!room.users.size) {
  ...
  #getNewRoomOwner(room, socket) {
    ...
    // se nao houver speakers, ele pega o attendee mais antigo (primeira posicao)
  ...
  #joinUserRoom(socket, user, room) {
    ...
    // definiar quem é o dono da sala
  ...
  #mapRoom(room) {
    ...
    const featureAttendees = users.slice(0, 3);
